# Murtaza Ali 🚀

### AI & Data Science Engineer | Data Analyst Intern | Python Developer

Focused software professional working at the intersection of **Technology, Code Logic, and Systems Engineering**. Leveraging Python to build efficient applications, automate workflows, and develop practical AI solutions.

---

### 💼 Professional Experience & Active Focus

* **Data Analyst Intern** | Remote *Arch_Technologies* *(June 2026 – Present)*
  * Data engineering, EDA, and optimized cleaning pipelines (NumPy, Pandas).
* **Python Programming Intern** | Remote *CodeAlpha* *(June 2026 – Present)*
  * Algorithm development, OOP, and web-based dashboard prototypes.
* **Proptech Integration**
  * Systematizing real estate data using advanced automation.

---

## 🌐 Socials:
[![LinkedIn](https://shields.io)](https://linkedin.com) [![X](https://shields.io)](https://x.com)

---

# 💻 Tech Stack:
![Python](https://shields.io) ![NumPy](https://shields.io) ![Pandas](https://shields.io) ![scikit-learn](https://shields.io) ![PyTorch](https://shields.io) ![TensorFlow](https://shields.io) ![Keras](https://shields.io) ![OpenCV](https://shields.io) ![Hugging Face](https://shields.io) ![LangChain](https://img.shields.io/badge/🦜🔗%20LangChain-1C3C3A?style=for-the-badge) ![Streamlit](https://shields.io) ![Matplotlib](https://shields.io) ![Plotly](https://shields.io) ![ggplot2](https://shields.io) ![Jupyter](https://shields.io) ![Git](https://shields.io) 

---

# 📊 GitHub Stats:
<p align="left">
  <img src="https://shion.dev" alt="Murtaza's GitHub Stats" width="48%" />
  <img src="https://shion.dev" alt="Top Languages" width="48%" />
</p>

---
<p align="right">
  <img src="https://komarev.com" alt="Visitor Count" />
</p>
